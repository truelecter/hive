# This file was generated by nvfetcher, please do not modify it manually.
{ fetchgit, fetchurl, fetchFromGitHub, dockerTools }:
{
  camera-streamer = {
    pname = "camera-streamer";
    version = "v0.2.6";
    src = fetchFromGitHub {
      owner = "ayufan";
      repo = "camera-streamer";
      rev = "v0.2.6";
      fetchSubmodules = true;
      sha256 = "sha256-m47fXOZuStaRPLJYGZKpg6RBPfFSkCafdqujagXTDXo=";
    };
  };
  klipper = {
    pname = "klipper";
    version = "8ef0f7d7e3d3b2ac7bc1e80ed3295ceca6bba4e7";
    src = fetchFromGitHub {
      owner = "Klipper3d";
      repo = "klipper";
      rev = "8ef0f7d7e3d3b2ac7bc1e80ed3295ceca6bba4e7";
      fetchSubmodules = false;
      sha256 = "sha256-f/fPnZvtnASphYtvM9NBae0on8GWSwQPykukZ3XCy3M=";
    };
    date = "2023-09-10";
  };
  klipper-ercf = {
    pname = "klipper-ercf";
    version = "fb9a6c7daaf881ab507368386e733a800b26f595";
    src = fetchFromGitHub {
      owner = "EtteGit";
      repo = "EnragedRabbitProject";
      rev = "fb9a6c7daaf881ab507368386e733a800b26f595";
      fetchSubmodules = false;
      sha256 = "sha256-Gwi0HiCIlTBQkHlFnmG0sZWB2xA1y5maJxF7VdfQMxU=";
    };
    date = "2022-09-02";
  };
  klipper-happy-hare = {
    pname = "klipper-happy-hare";
    version = "013bec683dd62f9a7b3928cfb8578533064b25b6";
    src = fetchFromGitHub {
      owner = "moggieuk";
      repo = "Happy-Hare";
      rev = "013bec683dd62f9a7b3928cfb8578533064b25b6";
      fetchSubmodules = false;
      sha256 = "sha256-SUAAntpCCQMT6Di5Fcpi9ljg5JYZzLZz8GvInMTechI=";
    };
    date = "2023-09-15";
  };
  klipper-kamp = {
    pname = "klipper-kamp";
    version = "v1.1.2";
    src = fetchFromGitHub {
      owner = "kyleisah";
      repo = "Klipper-Adaptive-Meshing-Purging";
      rev = "v1.1.2";
      fetchSubmodules = false;
      sha256 = "sha256-anBGjLtYlyrxeNVy1TEMcAGTVUFrGClLuoJZuo3xlDM=";
    };
  };
  klipper-klicky-probe = {
    pname = "klipper-klicky-probe";
    version = "f8dad75e072ed3cedd6e2179ff192917548deb00";
    src = fetchFromGitHub {
      owner = "jlas1";
      repo = "Klicky-Probe";
      rev = "f8dad75e072ed3cedd6e2179ff192917548deb00";
      fetchSubmodules = false;
      sha256 = "sha256-y0gy5aupACq7d/e+uFZk7ztiOQf4K8Mte/v0tWwRzyI=";
    };
    date = "2023-08-16";
  };
  klipper-led_effect = {
    pname = "klipper-led_effect";
    version = "35cf587fe958fe15a07c11b60564856582890460";
    src = fetchFromGitHub {
      owner = "julianschill";
      repo = "klipper-led_effect";
      rev = "35cf587fe958fe15a07c11b60564856582890460";
      fetchSubmodules = false;
      sha256 = "sha256-AZase17sO85Bp14DocUn+m6jT8P5ABXBJFdH7t4ANog=";
    };
    date = "2023-02-04";
  };
  klipper-screen = {
    pname = "klipper-screen";
    version = "77ecae70b2a81052e3c1f3dc54dd0097ffd4abfa";
    src = fetchFromGitHub {
      owner = "jordanruthe";
      repo = "KlipperScreen";
      rev = "77ecae70b2a81052e3c1f3dc54dd0097ffd4abfa";
      fetchSubmodules = false;
      sha256 = "sha256-oI6kyDwYG6Sh5qZkOjVAWjiG9d8U+XRYmUtUw+cQjSs=";
    };
    date = "2023-09-13";
  };
  klipper-z-calibration = {
    pname = "klipper-z-calibration";
    version = "v1.0.2";
    src = fetchFromGitHub {
      owner = "protoloft";
      repo = "klipper_z_calibration";
      rev = "v1.0.2";
      fetchSubmodules = false;
      sha256 = "sha256-QDsr09aIP09pI2r18atTRvbbMBS0rbjeiWOSnp9nRUk=";
    };
  };
  libdatachannel0_17 = {
    pname = "libdatachannel0_17";
    version = "v0.17.10";
    src = fetchFromGitHub {
      owner = "paullouisageneau";
      repo = "libdatachannel";
      rev = "v0.17.10";
      fetchSubmodules = false;
      sha256 = "sha256-3f84GxAgQiObe+DYuTQABvK+RTihKKFKaf48lscUex4=";
    };
  };
  libjuice = {
    pname = "libjuice";
    version = "v1.0.4";
    src = fetchFromGitHub {
      owner = "paullouisageneau";
      repo = "libjuice";
      rev = "v1.0.4";
      fetchSubmodules = false;
      sha256 = "sha256-LAqi5F6okhGj0LyJasPKRkUz6InlM6rbYN+1sX1N4Qo=";
    };
  };
  live555 = {
    pname = "live555";
    version = "2020.03.06";
    src = fetchTarball {
      url = "https://download.videolan.org/pub/contrib/live555/live.2020.03.06.tar.gz";
      sha256 = "sha256-UxunbrJC6UVlwexso/cuH9m+mUSf7WEPMLincUrqDWE=";
    };
  };
  mainsail = {
    pname = "mainsail";
    version = "c87586aebbdd6e7eb4b1505ce286d69955899f04";
    src = fetchFromGitHub {
      owner = "mainsail-crew";
      repo = "mainsail";
      rev = "c87586aebbdd6e7eb4b1505ce286d69955899f04";
      fetchSubmodules = false;
      sha256 = "sha256-b9HRCUmVg1Zrifg3z959Pga2f6ZJemR4wZDP6Lb2JiE=";
    };
    date = "2023-09-14";
  };
  mobileraker-companion = {
    pname = "mobileraker-companion";
    version = "fa9951318178e5f8a0acf7932e7d308ddaf638f4";
    src = fetchFromGitHub {
      owner = "Clon1998";
      repo = "mobileraker_companion";
      rev = "fa9951318178e5f8a0acf7932e7d308ddaf638f4";
      fetchSubmodules = false;
      sha256 = "sha256-AzrS8YBzTfFl82YlXz+vACudXNfCd0IdliZTHv58a88=";
    };
    date = "2023-09-09";
  };
  moonraker = {
    pname = "moonraker";
    version = "3d9052d711464cb5ea81cde99769c8f1fd7d3f8d";
    src = fetchFromGitHub {
      owner = "Arksine";
      repo = "moonraker";
      rev = "3d9052d711464cb5ea81cde99769c8f1fd7d3f8d";
      fetchSubmodules = false;
      sha256 = "sha256-4KHNmpeS0kunHrSh1vUFV8Hdz86kDRf79CXjpP/B4kk=";
    };
    date = "2023-08-14";
  };
  python-networkmanager = {
    pname = "python-networkmanager";
    version = "2.2";
    src = fetchurl {
      url = "https://pypi.org/packages/source/p/python-networkmanager/python-networkmanager-2.2.tar.gz";
      sha256 = "sha256-3m65IdlKunVJ9CjtKzqkgqXVQ+y2lly6oPu1VasxudU=";
    };
  };
}
